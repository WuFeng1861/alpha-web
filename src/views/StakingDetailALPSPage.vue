...
import {performStaking} from '../utils/useStakingALPS';

// 导入解除质押函数
import {performUnstaking} from '../utils/useStakingALPS';
...
const updateUserBalance = async (forceUpdate: boolean = false) => {
  ...
  try {
    ...
    myAlpsBalance.value = balances.alpsBalance;
    console.log('用户ALPHA余额更新完成:', balances.alpsBalance);
  } catch (error) {
...
const handleStakeAction = async () => {
  ...
  // 检查余额
  const userBalance = parseFloat(myAlpsBalance.value);
  if (userBalance < amount) {
    toast.error(t('staking.errors.insufficient_balance_alps', {
      balance: myAlpsBalance.value,
          ...
          <div class="space-y-3 mb-4">
            ...
            <div class="grid grid-cols-2 gap-3">
              <div class="bg-alpha-surface-light bg-opacity-90 rounded-lg p-4 border border-gray-700">
                ...
                <p class="text-white font-bold text-base break-all">{{ formatNumber(myAlpsBalance) }}</p>
              </div>
              <div class="bg-alpha-surface-light bg-opacity-90 rounded-lg p-4 border border-gray-700">
                <p class="text-gray-400 text-sm mb-1">{{ t('staking.annual_rate') }}</p>
